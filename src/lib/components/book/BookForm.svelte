...
<script>
    ...
    onMount(() => {
      ...
      // Use capture phase to avoid immediate submit issues
    ...
    function handleSubmit(event) {
      ...
      try {
        ...
        dispatch('toggleForm'); // Dispatch toggleForm to close the form
      } catch (err) {
...
>
  <button
    ...
    class="absolute top-2 right-2 sm:top-4 sm:right-4 p-2 text-gray-500 hover:bg-gray-200 hover:text-gray-700 rounded-full transition-all duration-200 z-10"
  >
  ...
  <form on:submit|preventDefault={handleSubmit} class="space-y-4 sm:space-y-6">
    <!-- Book Name and Author in one row -->
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div>
        <label for="title" class="block text-dark-green font-medium text-sm sm:text-base">Book Name</label>
        <input
          id="title"
          type="text"
          bind:value={title}
    ...
    <!-- How did you feel after reading the last page of the book? -->
    <div>
          ...
          {#each images as image, index}
            <div class="relative">
              ...
              <button
                ...
                class="absolute top-1 right-1 bg-red-500 text-white rounded-full w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center hover:bg-red-600 transition-all duration-200"
              >
    ...
    <button
      type="submit"
      class="w-full bg-dark-green text-white py-2 px-4 rounded-lg hover:bg-green-700 transition-all duration-200 text-sm sm:text-base"
    >
